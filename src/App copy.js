...
function App() {
  ...
  const [ageObj, setAgeObj] = useState({
	...
	let testNewDate = new Date(1887, 0, 1, 0, 0, 0, 0),
			testCurrentDate = new Date(),
			testDateDiff = testCurrentDate.getTime() - testNewDate.getTime(),
			testSecondsBetweenDates = testDateDiff / 1000,
			testMinutesBetweenDates = testSecondsBetweenDates / 60,
			testHoursBetweenDates = testMinutesBetweenDates / 60,
			testDaysBetweenDates = testHoursBetweenDates / 24,
			testMonthsBetweenDates = testDaysBetweenDates / 30.41666666,
			testYearsBetweenDates = testMonthsBetweenDates / 12;
...
testCurrentDate = ${testCurrentDate}
testDateDiff = ${testDateDiff}
testSecondsBetweenDates = ${testSecondsBetweenDates.toLocaleString('en-US', {maximumFractionDigits:2})}
testMinutesBetweenDates = ${testMinutesBetweenDates.toLocaleString('en-US', {maximumFractionDigits:2})}
testHoursBetweenDates = ${testHoursBetweenDates.toLocaleString('en-US', {maximumFractionDigits:2})}
testDaysBetweenDates = ${testDaysBetweenDates.toLocaleString('en-US', {maximumFractionDigits:2})}
testMonthsBetweenDates = ${testMonthsBetweenDates.toLocaleString('en-US', {maximumFractionDigits:2})}
testYearsBetweenDates = ${testYearsBetweenDates.toLocaleString('en-US', {maximumFractionDigits:2})}
					`)
				}
